 ...
 *
 * initialize the keyboard and allow keyboard interrupts
 * Inputs: None
 * Outputs: None
...
/* char_match
 *
 * match the input character to the scancode so that it's ready to be print
 * Inputs: c - input from keyboard 
 * Outputs: res - result of the converting
 * Side Effects: find the right character in scancode and then return
 */
 ...
 * Inputs: None
 * Outputs: None
 * Side Effects: read characters from keyboard everytime there is a keyboard interrupt
 * 					and then display in the screen
 */
...
outputs: count - number of bytes written
side effect: write buffers to terminal, mostly from syscall*/
int32_t terminal_write(char* buf, int count)
{
    cli();
	int idx=get_active_terminal();
    int lim = sizeof(print_buffer[idx]);
    if(count<lim) lim = count;
    memcpy( print_buffer[idx], buf, lim );
    print_buffer[idx][lim] = 0;

    puts_scroll((char*)print_buffer[idx],lim);
    sti();
    //putc_scroll('\n');
    return count;
}
...
side effect: read buffers from keyboard, mostly to syscall*/
int32_t terminal_read(char* buf, int count)
{
    sti();
	int idx=get_active_terminal();
    while(terminal_read_ready[idx]!=1)
    {
    }
    memcpy(buf, print_buffer[idx], count);
    terminal_read_ready[idx] = 0;
    return strlen(buf);
}
...
//when wrongly used in stdin and stdout, just return -1
int terminal_wrong(uint32_t inode, uint32_t offset, uint8_t* buf, uint32_t count){
	return -1;
}

//combination of terminal open and close. Just return 0, because we don't need open/close
